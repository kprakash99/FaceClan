...
public class face {
	...
	// move these to a method called coordinate math?
	...
	public void eyebrowAnalysis(Point[] leftbrow, Point[] rightbrow, Point[] leftEye, Point[] rightEye) {
		...
		lEyeAvg = centroid(leftEye);
		rEyeAvg = centroid(rightEye);
		lBrowAvg = centroid(leftbrow);
		rBrowAvg = centroid(rightbrow);

		// distance from eye to eyebrow
		lBrowDist = distance(lBrowAvg, lEyeAvg);
		rBrowDist = distance(rBrowAvg, rEyeAvg);
		// distance between left and right eyebrow centroids
		betweenBrowDist = distance(lBrowAvg, rBrowAvg);
		// distance between left and right eyebrow tips
		BrowTipDist = distance(leftbrow[leftbrow.length], rightbrow[0]);
	}
	public void lipMouthAnalysis() {
		
		upLipAvg = centroid(upperLip);
		loLipAvg = centroid(lowerLip);
		upLipAvg = centroid(upperLip);
		upLipAvg = centroid(upperLip);
	}
}
